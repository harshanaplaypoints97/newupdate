...
class ApiMarkert {
  ...
  Future<MarkertPlaceItem> createMarketplaceItem(
    ...
    try {
      ...
      Response response = await _dio.post(
        '$baseUrl/api/v1/marketplace-item-controller',
        data: {
          "item_name": itemName,
          "item_desc": itemDesc,
  ...
  Future<MarkertPlaceAllItem> GetMarkertPlaceItem(
      String category, int offset, int limit) async {
    try {
      await setCookie();

      Response response = await _dio.get(
          '$baseUrl/api/v1/marketplace-item-controller?offset=$offset&limit=$limit&category=$category');
      Logger().e(response.data.toString());
      return MarkertPlaceAllItem.fromJson(response.data);
    } catch (e) {
      if (e.response.statusCode == 400) {
        MarkertPlaceAllItem result = MarkertPlaceAllItem();
        result.message = 'Bad Request.';
        return result;
      } else if (e.response.statusCode == 401) {
        MarkertPlaceAllItem result = MarkertPlaceAllItem();
  ...
  Future<PlayerMarkertPlace> GetPlayerMarkertPlaceItem(
    ...
    try {
      await setCookie();
      Response response = await _dio.get(
          '$baseUrl/api/v1/marketplace-item-controller-player?offset=$offset&limit=$limit&category=All');
  ...
  Future<PlayerMarkertPlace> DeleatePlayerItem(String id) async {
    try {
      await setCookie();
      Response response =
          await _dio.delete('$baseUrl/api/v1/marketplace-item-controller/$id');
